...
MPCController::MPCController(int simSteps, double ph, State s0)
        : simulationSteps(simSteps), predictionHorizon(ph), firstState(s0)
{
}

void MPCController::runSimulation() // vector<State> track
{
...
vector<State> MPCController::readTrackFromFile(string filename) const
{
    fstream file;
...
Boundary MPCController::getTrackBoundary(State curState, Track track) const
{
