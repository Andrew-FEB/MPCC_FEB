...
/// Generated at: 2020-08-24 11:59:28.737702
///
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != MPCC_OPTIMIZER_NUM_DECISION_VARIABLES {
    ...
    if parameter.len() != MPCC_OPTIMIZER_NUM_PARAMETERS {
...
fn run_server() {
    ...
    let mut p = [0.0; MPCC_OPTIMIZER_NUM_PARAMETERS];
    let mut cache = initialize_solver();
    info!("Done");
    let listener = TcpListener::bind(format!("{}:{}", BIND_IP, BIND_PORT)).unwrap();
    let mut u = [0.0; MPCC_OPTIMIZER_NUM_DECISION_VARIABLES];
